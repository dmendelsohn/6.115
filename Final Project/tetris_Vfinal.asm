...
.equ BOUNDARY_STEP_SIZE, 03h	; boundary step size is 3
.equ LEFT_BUFFER_SIZE, 03h	; buffer on left side of size 3
...
.equ LONG_TIME_CONST, 120d		; every nth command isr, we do a timestep
...
refresh_display:
	...
	lcall draw_left_boundary
	lcall move_cursor_left_wall
	lcall move_cursor_right_wall
	lcall draw_right_boundary
	lcall move_cursor_right_wall
	lcall move_cursor_origin
...
;===============================================================
; draw_right_boundary - draws a vertical line on the right side
;		of the playing grid.
;===============================================================
draw_right_boundary:
	mov dptr, #XDAC
	...
	mov b, #BLOCKSIZE
	mul ab
	add a, #LEFT_BUFFER_SIZE
	movx @dptr, a			    ;set the x coordinate
	mov dptr, #YDAC				; point to YDAC
	mov a, #00h					; start a low
draw_right_boundary_loop:
	movx @dptr, a				; push new value	
	clr c
	add a, #BOUNDARY_STEP_SIZE
	jc finish_draw_right_boundary
	sjmp draw_right_boundary_loop
finish_draw_right_boundary:
	ret
	
;===============================================================
; draw_left_boundary - draws a vertical line on the left side
...
draw_left_boundary:
	...
	mov dptr, #YDAC				; point to YDAC
	mov a, #00h					; start a low
draw_left_boundary_loop:
	movx @dptr, a				; push new value	
	clr c
	add a, #BOUNDARY_STEP_SIZE
	jc finish_draw_left_boundary
	sjmp draw_left_boundary_loop
finish_draw_left_boundary:
	ret
	
;===============================================================
move_cursor_right_wall:
	mov a, #NUMBLOCKX
	mov b, #BLOCKSIZE
	mul ab
	mov b, #00h
...
get_coord_from_index:
	...
	mov b, #BLOCKSIZE
	mul ab
	add a, #LEFT_BUFFER_SIZE	; acc now holds x-coordinate
